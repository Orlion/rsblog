...
async fn main() -> Result<(), std::io::Error> {
    ...
    HttpServer::new(|| {
        App::new()
            .route("/index/{page}", web::get().to(handler::index_handler))
    })
    .bind(("127.0.0.1", 8080))?
    .run()
    .await
...
fn bootstrap() {
    // 初始化数据库连接池
    let pool = mysql::Pool::new("mysql://root:123456@localhost:3306/blog").expect("数据库连接池初始化失败");
    ...
    let controllers = Box::new(Controllers {
        index_controller,
    });

    unsafe { 
        CONTROLLERS = Some(Box::leak(controllers)) 
    };
}
